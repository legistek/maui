...
using System.Maui.Presentation.Data;
...
namespace System.Maui.Presentation.Controls
{
	...
	public sealed class ItemContainerGenerator : IRecyclingItemContainerGenerator //, IWeakEventListener
	{
		...
		/// </summary>
		ItemContainerGenerator IItemContainerGenerator.GetItemContainerGeneratorForPanel(Layout panel)
		{
		...
		{
			object item = container.GetValue(ItemForItemContainerProperty);
			...
			for (ItemBlock block = _itemMap.Next; block != _itemMap; block = block.Next)
			{
				if (index < block.ItemCount)
				{
					...
					object item = (Parent == null) && (container != null) ?
								container.GetValue(ItemForItemContainerProperty) : null;
		...
		internal static readonly BindableProperty ItemForItemContainerProperty =
				BindableProperty.CreateAttached(
					"ItemForItemContainer", 
					typeof(object), 
		...
		internal static void LinkContainerToItem(DependencyObject container, object item)
		{
			...
			if (container != item)
			{
				container.SetValue(View.BindingContextProperty, item);
			}
		}
		...
		internal static void UnlinkContainerFromItem(DependencyObject container, object item, IGeneratorHost host)
		{
			...
			if (container != item)
			{
				BindableProperty dp = View.BindingContextProperty;
				container.SetValue(dp, Compatibility.DisconnectedItem);
			}
		}
		...
		void OnItemRemoved(object item, int itemIndex)
		{
			...
			{
				GroupItem groupItem = (GroupItem)Peer;
				CollectionViewGroup group = groupItem.GetValue(ItemForItemContainerProperty) as CollectionViewGroup;
