...
using System.Maui.Presentation;
using System.Maui;
	...
	internal static class GlobalEventManager
	{
			...
			bool handledEventsToo)
		{
			Debug.Assert(
				...
				typeof(FrameworkElement).IsAssignableFrom(classType),
		...
		internal static void AddOwner(RoutedEvent routedEvent, Type ownerType)
		{
			...
			if ((ownerType == typeof(BindableObject)) || ownerType.IsSubclassOf(typeof(BindableObject)))
			{
			...
			out int index)
		{
			...
			if (classListenersLists == null)
			{
				...
				if (dType.SystemType == typeof(FrameworkElement))
				{
			...
			lock (Synchronized)
			{
				...
				if (_globalIndexToEventMap.Count >= Int32.MaxValue)
				{
					throw new InvalidOperationException(
						// SR.Get(SRID.TooManyRoutedEvents)
						);
				}
